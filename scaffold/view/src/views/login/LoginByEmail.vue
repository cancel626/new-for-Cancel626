    ...
    <div class="login-panel animated fadeIn">
      ...
      <div class="right-login">
        <transition-group name="slide-fade" tag="div">
          ...
          <div key="captcha" class="input-group captcha-group">
            <div class="captcha-wrapper">
              <input v-model="captchaCode" class="login-input captcha-input" placeholder="请输入验证码"
                @focus="animateInput('captcha')" @blur="resetInput('captcha')" maxlength="4" />
              <button class="send-captcha-btn" @click="sendCaptcha" :disabled="isSendingCaptcha">
          ...
          <div key="footer" class="login-footer">
            <p>其他方式: <span class="register-link" @click="toLoginByPassword">点击密码登录</span></p>
...
export default {
  ...
  methods: {
    async sendCaptcha() {
      if (!this.account) {
        this.$message.info('请先输入账号');
        return;
      }
      
      this.isSendingCaptcha = true;
      try {
        const userLoginDto = { account: this.account, captchaCode: this.captchaCode };
        // Call your API to send captcha
        await this.$axios.post(`user/sendEmail`, userLoginDto);
    ...
    async login() {
      if (!this.account || !this.captchaCode) {
        this.$message.info('账号与验证码不能为空哦');
        return;
      }
      ...
      const userLoginDto = { account: this.account,captchaCode: this.captchaCode };
      try {
        const { data, message } = await this.$axios.post(`user/loginByEmail`, userLoginDto);
